...
_CAPABILITY_KEYWORDS = [
    "能做什么", "能做些什么", "能干什么", "能干些什么", "能干嘛", "能干啥",
    "会什么", "会啥", "会做什么", "都会什么", "都会啥",
...
def is_capability_query(query: str) -> bool:
    ...
    识别策略（两级）：
      ...
      2. 宽松兜底：以"你/您"为主语 + 含能力关键词 + 不含法律关键词
         → 覆盖"你可以做什么""你能帮我干什么""你都能干嘛"等变体
    ...
    for pattern in _CAPABILITY_PATTERNS:
        if re.match(pattern, q):
            return True
    # 2. 宽松兜底：主语限定"你/您"，避免误伤"我能做什么""打架能做什么"
    if re.match(r'^[你您]', q):
        if any(kw in q for kw in _CAPABILITY_KEYWORDS):
            # 含法律关键词时视为法律问题（如"你能帮我查劳动法"），不拦截
            nq = _normalize(q)
